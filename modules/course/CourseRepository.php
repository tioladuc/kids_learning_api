...
class CourseRepository
{
    ...
    {
        $query = "SELECT count(*) AS total FROM learn4kids_visited_courses 
                    WHERE child_id = '$childId' AND course_code = '$courseCode' AND time_spent = 0 AND TIMESTAMPDIFF(HOUR, last_connection, NOW()) < 10 ";
        $stmt = $this->db->prepare($query);
        $stmt->execute([]);

        if($stmt->fetchAll(PDO::FETCH_ASSOC)[0]['total'] == 0) {
    ...
    public function urlCourseLeaving(string $childId, string $courseCode): bool
    {
        ...
        $stmt = $this->db->prepare($query);
        $stmt->execute([]);
        $data = $stmt->fetchAll(PDO::FETCH_ASSOC)[0]??null;
